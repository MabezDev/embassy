...
#[cfg(feature = "embedded-fatfs")]
mod embedded_fatfs_impl {
    use super::*;

    impl<'d, T: Instance, Dma: SdmmcDma<T>> embedded_fatfs::device::BlockDevice<512> for Sdmmc<'d, T, Dma> {
        type Error = Error;

        async fn read(&mut self, mut block_address: u32, buf: &mut [[u8; 512]]) -> Result<(), Self::Error> {
            assert!(&buf[0] as *const _ as u32 % 4 == 0);
        ...
        async fn write(&mut self, mut block_address: u32, buf: &[[u8; 512]]) -> Result<(), Self::Error> {
            assert!(&buf[0] as *const _ as u32 % 4 == 0);
